...
extension ButtonView {
    ...
    func subtractValues() {
        if count > 0 {
            ...
            orderVM.removeItem(itemName: item.name, count: count, price: item.price, total: order.total, image: order.image)
            addToCart = false
        }
    ...
    func neutralCount() {
        ...
        orderVM.addItem(itemName: item.name, count: count, price: item.price, total: order.total, image: item.image.thumbnail)
    }
